...
vi.mock("@/api/policies", () => ({
  ...
  createRawPolicy: (...args: unknown[]) => mockCreateRawPolicy(...args),
  deletePolicy: (...args: unknown[]) => mockDeletePolicy(...args),
  getPolicySource: (...args: unknown[]) => mockGetPolicySource(...args),
}));

vi.mock("@/constants/checkNames", () => ({
  getCheckName: (id: string) => `Name for ${id}`,
}));
...
describe("PoliciesPage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetPolicies.mockResolvedValue(mockPolicies);
  });

  it("renders page heading", async () => {
    renderPage();
    expect(screen.getByText("Rego Policies")).toBeInTheDocument();
  });
  ...
  it("shows severity badges", async () => {
    renderPage();
    await waitFor(() => {
      expect(screen.getAllByTestId("severity").length).toBeGreaterThanOrEqual(
        3,
      );
    });
  });
  ...
  it("shows no-match message when filter has no results", async () => {
    ...
    expect(
      screen.getByText("No policies match your search."),
    ).toBeInTheDocument();
  });
...
describe("PoliciesPage — tabs", () => {
  ...
  it("shows GUI Builder tab by default", () => {
    renderPage();
    expect(screen.getByText("GUI Builder")).toBeInTheDocument();
    expect(screen.getByText("Code Editor")).toBeInTheDocument();
    // GUI form should be visible
    expect(screen.getByPlaceholderText("s3_21")).toBeInTheDocument();
  });
  ...
  it("switches to Code Editor tab", () => {
    ...
    expect(
      screen.getByPlaceholderText("custom_check.rego"),
    ).toBeInTheDocument();
    expect(screen.getByText("Insert starter template")).toBeInTheDocument();
    // GUI form fields hidden
    expect(screen.queryByPlaceholderText("s3_21")).not.toBeInTheDocument();
  });

  it("switches back to GUI tab", () => {
    renderPage();
    fireEvent.click(screen.getByText("Code Editor"));
    fireEvent.click(screen.getByText("GUI Builder"));
    expect(screen.getByPlaceholderText("s3_21")).toBeInTheDocument();
  });
  ...
  it("inserts template in code editor", () => {
    ...
    fireEvent.click(screen.getByText("Insert starter template"));
    const textarea = document.querySelector(
      'textarea[name="rego_code"]',
    ) as HTMLTextAreaElement;
    expect(textarea.value).toContain("package aws.network");
    expect(textarea.value).toContain("violations contains");
  });
});
...
describe("PoliciesPage — GUI form preview", () => {
  ...
  it("shows Rego preview when required fields are filled", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Generated Rego Preview")).toBeInTheDocument();
    });
  });
});
...
describe("PoliciesPage — view source", () => {
  beforeEach(() => {
    ...
    mockGetPolicySource.mockResolvedValue({
      check_id: "iam_root_mfa",
      filename: "iam.rego",
      rego_code: 'package aws.identity.iam\n\n"check_id": "iam_root_mfa"',
    });
  });
  ...
  it("expands source code on view source click", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("iam.rego")).toBeInTheDocument();
    });
  });
});
...
describe("PoliciesPage — submit raw policy", () => {
  ...
  it("submits raw rego code", async () => {
    ...
    await waitFor(() => {
      expect(mockCreateRawPolicy).toHaveBeenCalledTimes(1);
    });
    const callArg = mockCreateRawPolicy.mock.calls[0][0];
    expect(callArg.domain).toBe("identity");
    expect(callArg.filename).toBe("my_check.rego");
    expect(callArg.rego_code).toContain("violations contains");
  });
});
