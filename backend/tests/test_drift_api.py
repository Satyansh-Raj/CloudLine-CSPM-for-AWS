...
class TestDriftFilterByAccount:
    """Filter by ?account_id= parameter."""

    ACCOUNT_A = "111111111111"
    ACCOUNT_B = "222222222222"
    REGION = "us-east-1"

    def _make_acct_state(self, account_id, check_id):
        return ViolationState(
            pk=f"{account_id}#{self.REGION}",
            sk=f"{check_id}#arn:aws:ec2:::sg/sg-1",
            check_id=check_id,
            status="alarm",
            previous_status="ok",
            severity="high",
    ...
    def setup_method(self):
        states_a = [
            self._make_acct_state(self.ACCOUNT_A, "check_a1"),
            self._make_acct_state(self.ACCOUNT_A, "check_a2"),
        ]
        states_b = [
            self._make_acct_state(self.ACCOUNT_B, "check_b1"),
        ]
        mgr = MagicMock()
        all_states = states_a + states_b
        mgr.query_by_status.side_effect = (
            lambda status, limit=100: (
                all_states if status == "alarm" else []
            )
        )
        mgr.query_by_check.return_value = all_states
        self._mock = mgr
        app.dependency_overrides[
            get_state_manager
        ] = lambda: self._mock
    ...
    def test_filter_by_account_id(self):
        """?account_id= returns only that account's alerts."""
        client = TestClient(app)
        resp = client.get(
            f"/api/v1/drift/alerts"
            f"?account_id={self.ACCOUNT_A}"
        )
        assert resp.status_code == 200
    ...
    def test_unknown_account_returns_empty(self):
        ...
        client = TestClient(app)
        resp = client.get(
            "/api/v1/drift/alerts?account_id=999999999999"
        )
        assert resp.status_code == 200
        assert resp.json()["alerts"] == []


class TestDriftAccountFilterWithType:
    """account_id combined with type filter."""

    ACCOUNT_A = "111111111111"
    ACCOUNT_B = "222222222222"
    REGION = "us-east-1"

    def _make_acct_state(self, account_id, check_id, status="alarm"):
        return ViolationState(
            pk=f"{account_id}#{self.REGION}",
            sk=f"{check_id}#arn:aws:ec2:::sg/sg-1",
            check_id=check_id,
            status=status,
            previous_status="ok" if status == "alarm" else "alarm",
            severity="high",
            risk_score=70,
            domain="network",
            resource_arn="arn:aws:ec2:::sg/sg-1",
    ...
    def setup_method(self):
        alarm_states = [
            self._make_acct_state(self.ACCOUNT_A, "check_a_alarm"),
            self._make_acct_state(self.ACCOUNT_B, "check_b_alarm"),
        ]
        ok_states = [
            self._make_acct_state(
                self.ACCOUNT_A, "check_a_ok", "ok"
            ),
        ]
        mgr = MagicMock()
        mgr.query_by_status.side_effect = (
            lambda status, limit=100: (
                alarm_states if status == "alarm" else ok_states
            )
        )
        self._mock = mgr
        app.dependency_overrides[
    ...
    def test_account_and_type_combined(self):
        """?account_id=A&type=new_violation returns A's alarms."""
        client = TestClient(app)
        resp = client.get(
            f"/api/v1/drift/alerts"
            f"?account_id={self.ACCOUNT_A}"
            f"&type=new_violation"
        )
        assert resp.status_code == 200
        alerts = resp.json()["alerts"]
        assert len(alerts) == 1
        assert alerts[0]["check_id"] == "check_a_alarm"
