...
# ── Batch 3D: classification-aware data_sensitivity ─
...
def _dc(
    ...
    """Build a DataClassification for tests."""
    _type_map = {
        "critical": ["pii"],
        "high": ["financial"],
        "medium": ["analytics"],
    ...
    return DataClassification(
        ...
        data_types=_type_map.get(sensitivity, []),
        confidence=confidence,
        source=source,
    )


class TestComputeDataSensitivityWithClassification:
    """compute_data_sensitivity() when passed a
    DataClassification (Batch 3D)."""

    def test_none_classification_uses_tag_fallback(
        self,
    ):
        """Without classification, tag-based path runs."""
        data = {
            "Tags": [
                {"Key": "data-classification",
                 "Value": "pii"},
        ...
        result = scorer.compute_data_sensitivity(
            data, classification=None
        )
        assert result == 100  # pii tag → 100

    def test_critical_macie_high_confidence(self):
        """critical + high confidence → 100."""
        dc = _dc("critical", confidence="high")
        result = scorer.compute_data_sensitivity(
            {}, classification=dc
        )
        assert result == 100

    def test_high_macie_high_confidence(self):
        """high sensitivity + high confidence → 80."""
        dc = _dc("high", confidence="high")
        result = scorer.compute_data_sensitivity(
            {}, classification=dc
        )
        assert result == 80
    ...
    def test_medium_high_confidence(self):
        """medium + high confidence → 50."""
        dc = _dc("medium", confidence="high")
        result = scorer.compute_data_sensitivity(
            {}, classification=dc
        )
        assert result == 50

    def test_low_high_confidence(self):
        """low + high confidence → 20."""
        dc = _dc("low", confidence="high")
        result = scorer.compute_data_sensitivity(
            {}, classification=dc
        )
        assert result == 20

    def test_critical_medium_confidence_scaled(self):
        """critical + medium confidence → 100 * 0.7 = 70."""
        dc = _dc(
            "critical",
            confidence="medium",
            source="heuristic",
    ...
    def test_high_medium_confidence_scaled(self):
        """high + medium confidence → 80 * 0.7 = 56."""
        dc = _dc(
            "high",
            confidence="medium",
            source="heuristic",
    ...
    def test_unknown_sensitivity_returns_default(self):
        """unknown sensitivity → DEFAULT_DATA_SENSITIVITY."""
        dc = _dc("unknown", confidence="high")
        result = scorer.compute_data_sensitivity(
            {}, classification=dc
        )
        assert result == DEFAULT_DATA_SENSITIVITY

    def test_classification_overrides_tags(self):
        """DataClassification takes priority over tags."""
        data = {
            "Tags": [
                {"Key": "data-classification",
                 "Value": "public"},  # tag says low
            ]
        }
        dc = _dc("critical", confidence="high")
        result = scorer.compute_data_sensitivity(
        ...
        # classification wins → 100, not tag value (10)
        assert result == 100

    def test_macie_source_high_confidence_full_score(
        self,
    ):
        """source=macie, confidence=high → no scaling."""
    ...
    def test_tag_source_high_confidence_full_score(
        self,
    ):
        """source=tag, confidence=high → no scaling."""
        dc = DataClassification(
            sensitivity="high",
    ...
    def test_heuristic_source_medium_confidence_scaled(
        self,
    ):
        """source=heuristic, confidence=medium → scaled."""
        dc = DataClassification(
            sensitivity="high",
            data_types=["financial"],
            confidence="medium",
        ...
        assert result == 56  # 80 * 0.7


class TestScoreMethodWithClassification:
    """score() threads classification to
    compute_data_sensitivity (Batch 3D)."""

    def test_score_accepts_classification_kwarg(self):
        """score() does not raise with classification=..."""
        v = _make_violation("high")
        dc = _dc("critical", confidence="high")
        # Must not raise
        dims = scorer.score(
            violation=v,
        ...
        assert isinstance(dims, RiskDimensions)

    def test_score_with_macie_classification_raises_sensitivity(
        self,
    ):
        """Macie critical classification → 100 data_sens."""
        v = _make_violation("high")
        dc = _dc("critical", confidence="high",
                 source="macie")
        dims = scorer.score(
    ...
    def test_score_no_classification_tag_fallback(self):
        """score() with no classification uses tags."""
        v = _make_violation("high")
        data = {
            "Tags": [
                {"Key": "data-classification",
    ...
    def test_score_heuristic_classification_scaled(self):
        """Heuristic medium confidence scales score."""
        v = _make_violation("high")
        dc = _dc(
            "critical",
            confidence="medium",
        ...
        assert dims.data_sensitivity == 70  # 100*0.7
