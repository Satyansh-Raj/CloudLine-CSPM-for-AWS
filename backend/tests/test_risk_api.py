...
class TestRiskScoresAccountFilter:
    """account_id query param scopes to that account."""
    ...
    def test_account_id_passed_to_query(self):
        """`account_id` param is forwarded to query_by_account."""
        client = TestClient(app)
        client.get(
            "/api/v1/risk/scores?account_id=111111111111"
        )
        self._mock.query_by_account.assert_called_once()
        call_args = self._mock.query_by_account.call_args
        assert call_args[0][0] == "111111111111"

    def test_default_account_from_settings(self):
        """Without account_id, settings.aws_account_id is used."""
        client = TestClient(app)
        client.get("/api/v1/risk/scores")
        self._mock.query_by_account.assert_called_once()
        call_args = self._mock.query_by_account.call_args
        # Should use the default account from settings
        assert call_args[0][0] != "111111111111"

    def test_account_id_returns_200(self):
        """account_id query param accepted without error."""
        client = TestClient(app)
        resp = client.get(
            "/api/v1/risk/scores?account_id=123456789012"
        )
        assert resp.status_code == 200


class TestRiskSummaryAccountFilter:
    """account_id query param scopes summary to that account."""
    ...
    def test_account_id_passed_to_summary_query(self):
        """`account_id` param forwarded to query_by_account."""
        client = TestClient(app)
        client.get(
            "/api/v1/risk/summary?account_id=222222222222"
        )
        self._mock.query_by_account.assert_called_once()
        call_args = self._mock.query_by_account.call_args
        assert call_args[0][0] == "222222222222"

    def test_account_id_summary_returns_200(self):
        """account_id query param on summary accepted."""
        client = TestClient(app)
        resp = client.get(
            "/api/v1/risk/summary?account_id=123456789012"
        )
        assert resp.status_code == 200
