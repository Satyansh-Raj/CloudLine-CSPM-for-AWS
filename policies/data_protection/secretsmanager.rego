...
# ---------------------------------------------------------------------------
# Rule secretsmanager_auto_rotation — Automatic rotation must be enabled
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	secret.rotation_enabled == false
	result := {
		"check_id": "secretsmanager_auto_rotation",
		"status": "alarm",
		"severity": "high",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_rotation_interval — Rotation interval must be <= 90 days
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	...
	result := {
		"check_id": "secretsmanager_rotation_interval",
		"status": "alarm",
		"severity": "high",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_no_public_access — Secret must not allow public access via resource policy
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	...
	not stmt.Condition
	result := {
		"check_id": "secretsmanager_no_public_access",
		"status": "alarm",
		"severity": "critical",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_kms_encryption — Secret must be encrypted with customer-managed KMS key
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_unused_cleanup — Secret must not be unused for > 90 days
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	secret.days_since_last_accessed > 90
	result := {
		"check_id": "secretsmanager_unused_cleanup",
		"status": "alarm",
		"severity": "medium",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_cross_account_org — Secret must not have cross-account access without
#                          org ID condition
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	...
	result := {
		"check_id": "secretsmanager_cross_account_org",
		"status": "alarm",
		"severity": "high",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_deletion_approval — Secret must not be scheduled for deletion without approval
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	...
	result := {
		"check_id": "secretsmanager_deletion_approval",
		"status": "alarm",
		"severity": "high",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_owner_purpose_tags — Secret must have owner and purpose tags
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	not secret.tags.owner
	result := {
		"check_id": "secretsmanager_owner_purpose_tags",
		"status": "alarm",
		"severity": "low",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_name_not_revealing — Secret name must not reveal its purpose or type
# ---------------------------------------------------------------------------
...
violations contains result if {
	...
	result := {
		"check_id": "secretsmanager_name_not_revealing",
		"status": "alarm",
		"severity": "low",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_rotation_validated — Rotation function must be validated (not orphaned)
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	secret.rotation_enabled == true
	...
	result := {
		"check_id": "secretsmanager_rotation_validated",
		"status": "alarm",
		"severity": "high",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_description_required — Secret description must not be empty
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	secret.description == ""
	result := {
		"check_id": "secretsmanager_description_required",
		"status": "alarm",
		"severity": "low",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_multi_region — Multi-region replication for critical secrets
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	...
	result := {
		"check_id": "secretsmanager_multi_region",
		"status": "alarm",
		"severity": "medium",
		"reason": sprintf(
...
# ---------------------------------------------------------------------------
# Rule secretsmanager_no_wildcard_principal — Wildcard principal in secret policy must be blocked
# ---------------------------------------------------------------------------
violations contains result if {
	some secret in input.secrets_manager.secrets
	some stmt in secret.resource_policy.Statement
	...
	stmt.Principal == "*"
	result := {
		"check_id": "secretsmanager_no_wildcard_principal",
		"status": "alarm",
		"severity": "critical",
		"reason": sprintf(
